...
int main(int argc, char* argv[]) {
  double* A = NULL;
  double* x = NULL;
  double* y_shared = NULL;
  int m, n;

  int thread_count;
  thread_count = strtol(argv[1], NULL, 10); // cmd line arg  

  Get_dims(&m, &n);
  
  A = malloc(m * n * sizeof(double) );
  x = malloc(n * sizeof(double) );
  y_shared = malloc(m * sizeof(double) );
  
  if (A == NULL || x == NULL || y_shared == NULL) {
...
# pragma omp parallel num_threads(thread_count)
   Mat_vect_mult(A, x, y_shared, m, n);

   Print_vector("y", y_shared, m);
